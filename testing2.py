...
def main():
	...
	def checks(address, name):
			...
			try:
				WebDriverWait(driver, timeout=1).until(lambda d: d.find_element(By.CSS_SELECTOR, 'span[style="color:#D93025"]'))
			...
			try:
				...
				flag = False
				for keyword in ["market", "store", "grocery", "supplier"]:
			...
			WebDriverWait(driver, timeout=1).until(lambda d: d.find_element(By.CLASS_NAME, title))
			...
			try:
				WebDriverWait(driver, timeout=1).until(lambda d: d.find_element(By.CSS_SELECTOR, reviews))
				restaurant_reviews = driver.find_element(By.CSS_SELECTOR, reviews).text.lower().split('\n')
			except Exception as e:
				print("Could not get/find reviews")

			try:
				WebDriverWait(driver, timeout=1).until(lambda d: d.find_element(By.CSS_SELECTOR, description))
	...
	while i <= 2527:
		...
		try:
			...
			try:
				WebDriverWait(driver, timeout=1).until(lambda d: d.find_element(By.CLASS_NAME, "hfpxzc"))
